...
export async function getWeatherByCityService({ latitude, longitude }: SearchCityWeatherProps): Promise<WeatherResponseProps> {
  ...
  const today = {
    weather: {
      temp: `${Math.ceil(main.temp)}ºF`,
      temp_min: `${Math.floor(main.temp_min)}ºF`,
      temp_max: `${Math.ceil(main.temp_max)}ºF`,
      description: weather[0].description,
      details: weatherIcons[weather[0].main],
    },
    details: {
      feels_like: `${Math.floor(main.feels_like)}ºF`,
  ...
  data.list.forEach((item: any) => {
    ...
    if (days.includes(day) && !daysAdded.includes(day)) {
      ...
      nextDays.push({
        ...
        min: `${Math.floor(item.main.temp_min)}ºF`,
        max: `${Math.ceil(item.main.temp_max)}ºF`,
